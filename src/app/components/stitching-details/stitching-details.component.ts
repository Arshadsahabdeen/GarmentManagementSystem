...
export class StitchingDetailsComponent implements OnInit {
  ...
  validateStitchingForm(): boolean {
    ...
    if (d.Size === undefined || d.Size === null || d.Size <= 0) {
      ...
      this.setError(
        'Size',
        `Size cannot exceed available quantity (${this.selectedMaterialQty} m).`
      );
    }
    if (!d.Stitching_Date) {
      ...
      if (stitchingDate > today) {
        this.setError(
          'Stitching_Date',
          'Stitching date cannot be in the future.'
        );
      } else if (stitchingDate < twoWeeksAgo) {
        this.setError(
          'Stitching_Date',
          'Stitching date cannot be older than 2 weeks.'
        );
      }
    }
    ...
    if (
      d.Quantity_Stitched === undefined ||
      d.Quantity_Stitched === null ||
      d.Quantity_Stitched <= 0
    ) {
    ...
    if (
      d.Quality_Check_Status === undefined ||
      d.Quality_Check_Status === null
    ) {
      this.setError(
        'Quality_Check_Status',
        'Quality check status is required.'
      );
    }
  ...
  onSizeChange() {
    this.newDetails.Quantity_Stitched = this.calculateQuantityStitched(
      this.newDetails.Size
    );
  }
  ...
  printTable(): void {
    const printContents =
      document.getElementById('stitchingPrintArea')?.innerHTML;
